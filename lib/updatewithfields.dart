...
class Addandupdate extends StatefulWidget {
  ...
  const Addandupdate({Key? key, required this.auditData}) : super(key: key);
...
class _addandupdateState extends State<Addandupdate> {
  ...
  void initState() {
    super.initState();
    // Set initial values for text controllers using fetched data
    auditno.text = widget.auditData['auditReportNo'] ?? '';
    taxpayername.text = widget.auditData['nameOfTheTaxpayer'] ?? '';
    email.text = widget.auditData['email'] ?? '';
    taxpayeraddress.text = widget.auditData['addressOfTheTaxpayer'] ?? '';
    rangeanddivision.text = widget.auditData['rangeAndDivision'] ?? '';
    regno.text = widget.auditData['registrationNumberOfTheTaxpayer'] ?? '';
    startdate.text = widget.auditData['startDate'] ?? '';
    enddate.text = widget.auditData['endDate'] ?? '';
    dateofau.text = widget.auditData['dateOfAudit'] ?? '';
    iapno.text = widget.auditData['IAPno'] ?? '';
    nameofsupdt.text = widget.auditData['Nameofsupdt'] ?? '';
    objections = List<String>.from(widget.auditData['objectionInBriefForEachPara'] ?? []);
    amounts = List<String>.from(widget.auditData['duty'] ?? []);
    total.text = widget.auditData['total'] ?? '';
    spot.text = widget.auditData['spotrecovery'] ?? '';
    amountother.text = widget.auditData['taxrecoverd'] ?? '';
    divno.text = widget.auditData['divisionalFileNumber'] ?? '';
    SCNno.text = widget.auditData['scnNo'] ?? '';
    SCNdate.text = widget.auditData['scnDate'] ?? '';
    closurepara = List<String>.from(widget.auditData['reasonsForClosureOfPara'] ?? []);
    dateofclo.text = widget.auditData['dateClosureOfPara'] ?? '';
    remarks.text = widget.auditData['remarks'] ?? '';
    amtscn.text = widget.auditData['amountDemandedInScn'] ?? '';
    // Initialize other text controllers with respective data
  }
  ...
  void _auditdate(BuildContext context) async {
    ...
    if (picked != null) {
      setState(() {
        dateofau.text = DateFormat("yyyy-MM-dd").format(picked);
      });
    }
  }
